...
departments = ['meat-poultry', 'fruit-veg', 'fish-seafood', 'fridge-deli', 'bakery', 'frozen', 'pantry', 'beer-wine', 'drinks', 'health-beauty', 'household', 'baby-child', 'pet']
        ...
        for item in items:
            if item['type'] != 'PromoTile':
                try:
                    ...
                    product['price'] = str(item['price']['originalPrice'])
                    product['promoprice'] = str(item['price']['salePrice'])
                    product['priceMode'] = item['unit']
                    product['volumeSize'] = item['size']['volumeSize']
                    product['sku'] = item['sku']
                    product['dept'] = item['departments'][0]['name'] if item.get('departments') and len(item['departments']) > 0 else 'Unknown'

                    products_output.append(product)
                except:
                    continue
        print(f"Scraping department: {department}, page: {page}")
        page = page + 1
